...
class ImportCryptocurrenciesQuotes implements ShouldQueue
{
    ...
    {
        try {
            $listing = $cryptocurrencyService->getListing();
            foreach ($listing->cryptocurrencies as $currency) {
                foreach ($currency->quotes as $quote) {
                    try {
                        /** @var CryptocurrencyExternalId $cryptocurrencyExternalId */
                        $cryptocurrencyExternalId = CryptocurrencyExternalId::where('value', $currency->external_id)
